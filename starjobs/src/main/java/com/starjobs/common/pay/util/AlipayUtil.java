package com.starjobs.common.pay.util;
...
public class AlipayUtil {
	...
	static {
		try {
			Resource resource = new ClassPathResource("alipay/alipay_private_key_pkcs8.pem");
			APP_PRIVATE_KEY = FileUtil.readInputStream2String(resource.getInputStream());
			resource = new ClassPathResource("alipay/alipay_public_key.pem");
		...
		if (alipayClient == null) {
			synchronized (AlipayUtil.class) {
				if (null == alipayClient) {
					alipayClient = new DefaultAlipayClient("http://openapi.alipay.com/gateway.do", ALIPAY_APPID,
